...
try {
  // @ts-ignore
  MorphSVGPlugin =
    require('gsap/MorphSVGPlugin').MorphSVGPlugin ||
    (window as any).MorphSVGPlugin;
} catch (e) {
...
function initInteractiveElements(): void {
  ...
  const buttons = Array.from(
    document.querySelectorAll('.btn-animated')
  ) as HTMLElement[];
...
function animateHeroSection(): void {
  ...
  const wordInners = Array.from(
    heroSection.querySelectorAll('.word-inner')
  ) as HTMLElement[];
  const heroDesc = heroSection.querySelector(
    '.hero-description'
  ) as HTMLElement | null;
  ...
  const scrollIndicator = heroSection.querySelector(
    '.scroll-indicator'
  ) as HTMLElement | null;
  ...
  const featureIcon = document.querySelector(
    '.feature-svg'
  ) as HTMLElement | null;
...
function initTestimonialCarousel(): void {
  ...
  carousel.addEventListener('focusout', (e) => {
    if (!carousel.contains(e.relatedTarget as Node)) startInterval();
  });
